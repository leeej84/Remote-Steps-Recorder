                     ...
                     if( $retval -and $ppQueryInfo )
                     {
                        ...
                        if( $value -and $value.Data -and $value.Data.WTSInfoExLevel1.SessionState -ne [WTS_CONNECTSTATE_CLASS]::WTSListen -and $value.Data.WTSInfoExLevel1.SessionState -ne [WTS_CONNECTSTATE_CLASS]::WTSConnected )
                        {
                            ...
                            Add-Member -InputObject $wtsinfo -Force -NotePropertyMembers @{
                                'IdleTimeInSeconds' =  $idleTime | Select-Object -ExpandProperty TotalSeconds
                                'IdleTimeInMinutes' =  $idleTime | Select-Object -ExpandProperty TotalMinutes
...
Function RunRemoteTask {
    ...
    #Remove the trailing \ if one has been added
    if ("\" -eq $location.Substring(0,$location.Length-1)) {
...
Function RefreshStatus {
    ...
    if ($PSRInstance) {
        ...
        $wpf.txtRecorderStatus.Text = "PSR Recorder is not running"
    ...
    if ($wpf.txtRemoteManage.Text -eq "Not Remotely Manageable") {
        ...
        $wpf.txtNetworkLocation.IsEnabled = $false
    } else {        
        $wpf.txtRemoteManage.Foreground="green"
        #Add users to the list
        $wpf.lstLoggedInUsers.Items.Clear()        
        foreach ($user in (Get-WTSSessionInformation -Computer $wpf.lstADComputers.SelectedItem | Select-Object -ExpandProperty UserName)) {$wpf.lstLoggedInUsers.Items.Add($user)}
        $wpf.txtNetworkLocation.IsEnabled = $true
        #Button not enabled because the username selection is wiped
        $wpf.btnRunStepsRecorder.IsEnabled = $false
    }
...
<Window
    ...
    <Grid Margin="1,0,29,36">
        <ListBox Name="lstADComputers" Margin="10,46,595,95"/>
        <ListBox Name="lstLoggedInUsers" Margin="200,46,400,95"/>    
        ...
        <Button Name="btnRefreshStatus" Content="Refresh Status" HorizontalAlignment="Left" Margin="400,300,0,0" VerticalAlignment="Top" RenderTransformOrigin="0.25,0.607" Width="120" Height="43" AutomationProperties.HelpText="Click here to refresh the and recheck the machine"/>
        <Button Name="btnRemoteConnect" Content="Remote Desktop" HorizontalAlignment="Left" Margin="521,300,0,0" VerticalAlignment="Top" Height="43" Width="120" AutomationProperties.HelpText="Stop the steps recorder running to produce the data"/>
        <Button Name="btnSaveLocation" Content="Open Save &#xD;&#xA;Location" HorizontalAlignment="Left" Margin="642,300,0,0" VerticalAlignment="Top" RenderTransformOrigin="0.25,0.607" Width="120" Height="43" AutomationProperties.HelpText="Click to open the folder share the capture will be stored in"/>
        <Button Name="btnRunStepsRecorder" Content="Run Steps Recorder" HorizontalAlignment="Left" Margin="400,344,0,0" VerticalAlignment="Top" Height="43" Width="120" AutomationProperties.HelpText="Run the steps recorder, will be disable if the machine is not remotely manageable"/>
        <Button Name="btnStopStepsRecorder" Content="Stop Steps Recorder" HorizontalAlignment="Left" Margin="521,344,0,0" VerticalAlignment="Top" Height="43" Width="120" AutomationProperties.HelpText="Stop the steps recorder running to produce the data"/>
        <Button Name="btnClose" Content="Close" HorizontalAlignment="Left" Margin="642,344,0,0" VerticalAlignment="Top" Height="43" Width="120"/>       
        ...
        <Label Content="Active Directory Computers" HorizontalAlignment="Left" Margin="10,20,0,0" VerticalAlignment="Top" Width="181"/>        
        <Label Content="Users" HorizontalAlignment="Left" Margin="200,20,0,0" VerticalAlignment="Top" Width="158"/>
        ...
        <TextBox Name="txtNetworkLocation" HorizontalAlignment="Left" Margin="400,260,0,0" TextWrapping="Wrap" VerticalAlignment="Top" Width="360" Height="31" AutomationProperties.HelpText="Make sure the location is writeable by the user and machine you are capturing from.">
            <TextBox.Effect>
                <DropShadowEffect/>
            </TextBox.Effect>
        </TextBox>
        ...
        <TextBlock HorizontalAlignment="Left" Margin="10,394,0,-25" TextWrapping="Wrap" Width="760" RenderTransformOrigin="0.501,1.138"><Run Text="This tool will remote launch the steps recorder on a machine for a user and capture their activity, this activity will then be uploaded to a fileshare for review. Pick a machine, pick a user, run the recorder, all activity is recorder, stop the recorder and open the network location."/><LineBreak/><Run/></TextBlock>
...
#On update of username selected
$wpf.lstLoggedInUsers.add_SelectionChanged({
    if($wpf.txtNetworkLocation.Text) {
        if ((Test-Path $wpf.txtNetworkLocation.Text) -and ($wpf.txtRemoteManage.Text -eq "Remotely Manageable")) {
            $wpf.btnRunStepsRecorder.IsEnabled = $true
        }
        if ($wpf.txtRemoteManage.Text -eq "Not Remotely Manageable") {
            $wpf.btnRunStepsRecorder.IsEnabled = $true
...
$wpf.btnRemoteConnect.add_Click({
    Start-Process -FilePath Mstsc.exe -ArgumentList "/v:$($wpf.lstADComputers.SelectedItem)"
})
